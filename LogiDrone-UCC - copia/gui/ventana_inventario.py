...
# Reemplaza completamente el archivo original.
# Mejoras:
#   · Raíz en cian con halo glow
#   · Sub-árbol izquierdo en azul, derecho en púrpura
#   · Hojas diferenciadas (verde neón + rombo)
#   · Rectángulos de zona para cada subárbol
#   · Badge FE codificado en color (verde / amarillo / rojo)
#   · Panel lateral inspector del nodo seleccionado
#   · Zoom con rueda + pan con arrastre
#   · Hover: nodo se ilumina y borde engrosa
...
class VentanaInventario(tk.Toplevel):
    def __init__(self, parent, sistema):
        ...
        self._drag_x    = 0
        self._drag_y    = 0
        self._selected  = None          # id del nodo seleccionado
    ...
    def _build_ui(self):
        ...
        right = tk.Frame(body, bg=C_SURF, width=190, padx=12, pady=12)
    ...
    def _build_form(self, parent):
        ...
        for label, attr, ph in campos:
            ...
            e = tk.Entry(parent, bg=C_SURF2, fg=C_WHITE, insertbackground=C_CYAN,
                         relief=tk.FLAT, font=('Courier New', 9), width=22,
        ...
        ttk.Combobox(parent, textvariable=self.var_cat, width=20,
        ...
        tk.Button(row, text='⌕ BUSCAR', bg=C_SURF2, fg=C_CYAN,
                  ...
                  command=self._buscar).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0,4), ipady=3)
        tk.Button(row, text='✖ ELIMINAR', bg=C_SURF2, fg=C_RED,
                  font=('Courier New', 7), relief=tk.FLAT, cursor='hand2',
                  highlightthickness=1, highlightbackground=C_RED,
                  command=self._eliminar).pack(side=tk.LEFT, fill=tk.X, expand=True, ipady=3)
        ...
        style.configure('Inv.Treeview',
                        ...
                        font=('Courier New', 8), rowheight=22, borderwidth=0)
        ...
        for col, w in zip(cols, [38, 90, 42]):
    ...
    def _build_canvas(self, parent):
        ...
        for txt, delta in [('−', -0.15), ('+', 0.15)]:
            tk.Button(ctrl, text=txt, bg=C_SURF2, fg=C_TEXT,
                      ...
                      highlightthickness=1, highlightbackground=C_BORDER,
                      command=lambda d=delta: self._zoom(d)).pack(side=tk.LEFT, padx=2, ipady=1, ipadx=4)
    ...
    def _load_table(self):
        ...
        for p in self.sistema.lista_productos():
            ...
            self.tree_tbl.insert('', 'end',
                                  values=(p.id_producto, p.nombre[:12], p.stock),
    ...
    def _draw_tree(self):
        ...
        # ── Identificar qué lado corresponde a cada nodo ──
        ...
        if left_ids:  zona_rect(left_ids,  AVL_LEFT)
        if right_ids: zona_rect(right_ids, AVL_RIGHT)

        # Etiquetas de subárbol
        ...
        if left_ids:  zona_label(left_ids,  AVL_LEFT,  'LEFT SUBTREE')
        if right_ids: zona_label(right_ids, AVL_RIGHT, 'RIGHT SUBTREE')
        ...
        for n in nodos:
            ...
            if is_sel:
                c.create_oval(x-r-9, y-r-9, x+r+9, y+r+9,
                              fill='', outline=color, width=1.5, dash=(5, 3))

            # Cuerpo
            tag = f'node_{n["id"]}'
            c.create_oval(x-r, y-r, x+r, y+r,
                          fill=C_SURF2, outline=color,
                          width=2.2 if is_root else 1.5, tags=(tag,))

            # Texto ID
            font_sz = int(14 * self._scale) if is_root else int(11 * self._scale)
            font_sz = max(8, min(font_sz, 16))
            ...
            # Guardar meta para el inspector y el click
            ...
            self._node_meta[n['id']] = {
                'color': color, 'cx': x, 'cy': y, 'r': r,
                'nombre': n['nombre'], 'tipo': tipo_lbl,
                'nivel': n['nivel'], 'fe': fe, 'is_leaf': is_leaf,
            }

            # Eventos de hover + click por tag
            c.tag_bind(tag, '<Enter>',
                        lambda e, t=tag, nc=C_SURF, col=color:
                            self.canvas.itemconfigure(t, fill='#12233A'))
            c.tag_bind(tag, '<Leave>',
                        lambda e, t=tag: self.canvas.itemconfigure(t, fill=C_SURF2))
    ...
    def _hex_alpha(hex_color, alpha):
        """Convierte #RRGGBB + alpha a color mezclado sobre C_SURF2."""
    ...
    def _insertar(self):
        try:
            ...
            messagebox.showwarning('⚠ ERROR',
                'ID, stock y peso deben ser números.', parent=self)
            return
        if not nombre:
    ...
    def _eliminar(self):
        ...
        if not messagebox.askyesno('CONFIRMAR',
                f'¿Eliminar producto {id_p} del árbol AVL?', parent=self):
            return
