...
# CAMBIOS v2:
#   - ZIRUMA: reubicado al centro del triángulo Centro-Taganga-Recarga Norte
#             (lat 11.2530, lon -74.1960) → zona excluida real
#   - RECARGA_N: reubicada al NOR-ESTE (lat 11.2730, lon -74.1800)
#                fuera del radio de Ziruma, accesible desde Taganga y Almacén
#   - AEROPUERTO: sigue excluido (zona aérea controlada)
#   - Las rutas CENTRO<->TAGANGA y CENTRO<->RECARGA_N pasan CERCA de Ziruma
#     pero el grafo no las conecta directamente → A* debe rodear por RECARGA_N
def construir_mapa():
    g = Grafo()
    ...
    # ZIRUMA: ahora en el centro del triángulo Centro-Taganga-Recarga Norte
    # Coordenadas = centroide aproximado de ese triángulo
    ...
    # ── Aristas (rutas aéreas que EVITAN Ziruma) ───────────────────────────────
    # El dron rodea Ziruma usando RECARGA_N como punto de paso seguro.
    # No existe conexión directa Centro<->Taganga ni Centro<->RECARGA_N (vieja)
    # porque esas rutas pasarían por la zona excluida de Ziruma.

    # Desde Almacen
    g.agregar_arista('ALMACEN',   'CENTRO')        # directo, no toca Ziruma
    g.agregar_arista('ALMACEN',   'BELLO')
    g.agregar_arista('ALMACEN',   'RECARGA_S')
    g.agregar_arista('ALMACEN',   'RECARGA_N')     # ruta larga pero segura

    # Recarga Norte (nueva posición NE) conecta Taganga y Almacen
    g.agregar_arista('RECARGA_N', 'TAGANGA')       # bordeando Ziruma por el norte
    g.agregar_arista('RECARGA_N', 'CENTRO')        # bordeando Ziruma por el este
    ...
    # NOTA: NO hay arista Centro<->Taganga directa porque cruzaría Ziruma.
    # La ruta Centro -> Taganga debe pasar por RECARGA_N (rodeo):
    #   Centro → RECARGA_N → Taganga  (A* la encontrará automáticamente)
