...
// ==========================================
// 1. ZONAS Y GRAFO (Usando tu nueva paleta)
// ==========================================
function cargarGrafoZonas() {
    fetch('/api/grafo').then(res => res.json()).then(data => {
        // Zona Ziruma
        L.circle([data.ziruma.centro[0], data.ziruma.centro[1]], {color: '#86868B', fillColor: '#131313', fillOpacity: 0.2, radius: data.ziruma.radio_km*1000}).addTo(map);
        
        // Rutas
        data.aristas.forEach(a => L.polyline([a.origen, a.destino], {color: '#86868B', weight: 2, opacity: 0.6, dashArray: '4, 6'}).addTo(map));
        
        // Nodos
        data.nodos.forEach(n => L.circleMarker([n.lat, n.lon], {radius: 6, color: '#131313', fillColor: '#1561F0', fillOpacity: 1, weight: 2}).addTo(map).bindPopup(`<b>${n.nombre}</b>`));
    });
}
...
function actualizarPantallaPedidos() {
    ...
    fetch('/api/drones').then(res => res.json()).then(data => {
        ...
        data.drones.forEach(d => {
            if(d.estado === 'en_espera') select.innerHTML += `<option value="${d.id}">${d.id} (Batería: ${d.bateria}%)</option>`;
        });
    });
}
...
function crearPedido(e) {
    ...
    fetch('/api/pedidos', { 
        ...
        body: JSON.stringify({
            destino: document.getElementById('ped-destino').value, tipo: document.getElementById('ped-tipo').value, 
            prioridad: document.getElementById('ped-prioridad').value, peso: document.getElementById('ped-peso').value
        }) 
...
function despacharViaje() {
    ...
    fetch('/api/pedidos/despachar', { method: 'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({id_dron: dronId}) })
    .then(res => res.json()).then(data => {
        if(data.status === 'success') {
            alert("ALGORITMO A* COMPLETADO:\n\n" + data.message);
            actualizarPantallaPedidos(); actualizarPantallasDrones();
        } else { alert("ERROR TÁCTICO:\n" + data.message); }
    });
}
...
function actualizarPantallaInventario() {
    fetch('/api/inventario/lista').then(res => res.json()).then(data => {
        ...
        tabla.innerHTML = `<tr><th>ID</th><th>Nombre</th><th>Stock</th><th>Peso (Kg)</th></tr>`;
        data.productos.forEach(p => tabla.innerHTML += `<tr><td>${p.id}</td><td>${p.nombre}</td><td><span style="color:#1561F0; font-weight:bold;">${p.stock}</span> Unds</td><td>${p.peso}</td></tr>`);
    });

    fetch('/api/inventario/arbol').then(res => res.json()).then(data => {
        let str = "RAÍZ (Autobalanceado)\n";
        if(data.nodos.length === 0) str += "└── (Árbol Vacío)";
        data.nodos.forEach(n => {
            let indent = "   ".repeat(n.nivel);
            let icono = (n.fe === 0 && n.nivel > 0) ? "📄" : "📁"; 
            str += `${indent}├── ${icono} [ID: ${n.id}] ${n.nombre} (FE: ${n.fe})\n`;
        });
        document.getElementById('avl-visualizer').innerText = str;
    });
}
...
function agregarInventario(e) {
    ...
    fetch('/api/inventario', {
        method: 'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({
            id: document.getElementById('inv-id').value, nombre: document.getElementById('inv-nom').value, 
            tipo: document.getElementById('inv-tipo').value, stock: document.getElementById('inv-stock').value, peso: document.getElementById('inv-peso').value
        })
    }).then(() => { actualizarPantallaInventario(); document.getElementById('form-inventario').reset(); });
}
...
function actualizarPantallasDrones() {
    fetch('/api/drones').then(res => res.json()).then(data => {
        ...
        data.drones.forEach(d => {
            ...
            tabla.innerHTML += `<tr><td>${d.id}</td><td><span style="color:#86868B;">${d.modelo}</span></td><td style="color:#1561F0; font-weight:bold;">${d.estado.toUpperCase()}</td><td style="color:${colorBat}; font-weight:bold;">${d.bateria}%</td></tr>`;
...
function recargarBateria() {
    ...
    fetch(`/api/drones/${id}/recargar`, {method: 'POST'}).then(() => { alert("Batería al 100%"); actualizarPantallasDrones(); });
}

function completarEntrega() {
    const id = document.getElementById('ctrl-dron-id').value;
    fetch(`/api/drones/${id}/completar`, {method: 'POST'}).then(res => res.json()).then(data => { alert("ORDEN EMITIDA:\n" + data.message); actualizarPantallasDrones(); });
}
...
function registrarMantenimiento() {
    ...
    fetch(`/api/drones/${id}/mantenimiento`, { method: 'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({tecnico: tec, observacion: obs}) })
    .then(res => res.json()).then(data => {
        alert(data.message); document.getElementById('ctrl-tecnico').value = ''; document.getElementById('ctrl-obs').value = '';
    });
}

function verHistorialPila() {
    const id = document.getElementById('ctrl-dron-id').value;
    fetch(`/api/drones/${id}/pila`).then(res => res.json()).then(data => { alert(`CIMA DE LA PILA (Último Mantenimiento):\n\n${data.ultimo_registro}`); });
}

// ==========================================
// 5. TELEMETRÍA DINÁMICA (Drones en color #1561F0)
// ==========================================
const ws = new WebSocket(`ws://${window.location.host}/ws/telemetry`);

// El dron ahora usa el color azul principal de tu paleta
...
ws.onmessage = function(event) {
    ...
    Object.keys(dronesMarkers).forEach(id => {
        if(!idsEnVuelo.includes(id)) { 
            ...
            delete dronesMarkers[id]; delete dronesRoutes[id]; 
        }
    });
    ...
    dronesEnVuelo.forEach(data => {
        ...
        if(!dronesMarkers[data.id]) {
            ...
            // La ruta del dron se dibuja en gris medio para no recargar visualmente el mapa
