// avl_visualizer.js — VERSIÓN CORREGIDA con eliminación de productos
'use strict';
...
function avlSelect(id){
  ...
  panel.innerHTML=`<div style="border-left:3px solid ${col};padding-left:10px;margin-bottom:12px">
    ...
    <button onclick="eliminarProductoDesdeArbol(${n.id},'${(n.nombre||'').replace(/'/g,"\\'")}')"
      style="margin-top:14px;width:100%;background:transparent;border:1px solid #FF4D6D;
             color:#FF4D6D;font-family:monospace;font-size:11px;font-weight:bold;
             padding:6px;border-radius:4px;cursor:pointer;letter-spacing:.05em"
      onmouseover="this.style.background='#FF4D6D';this.style.color='#07111F'"
      onmouseout="this.style.background='transparent';this.style.color='#FF4D6D'">
      ✖ ELIMINAR NODO ${n.id}
    </button>`;
}

// Función llamada desde el botón del inspector del árbol
function eliminarProductoDesdeArbol(id, nombre) {
...
// ── actualizarPantallaInventario — VERSIÓN CON BOTÓN ELIMINAR ─────────────
// Esta función reemplaza a la de map.js. Incluye la columna Acción con
// botón "✖ Eliminar" que llama a eliminarProducto() definida en map.js.
      ...
      (data.productos||[]).forEach(p=>{
        ...
        tr.innerHTML=`
          ...
          <td style="text-align:center">
            <button
              onclick="eliminarProducto(${p.id},'${nombre_safe}')"
...
// Override del form de inserción para refrescar tabla correctamente
