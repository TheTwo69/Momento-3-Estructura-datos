...
async def api_inventario_lista():
    """
    BUGFIX: Producto usa .peso_kg, no .peso.
    Se usa getattr con fallback para máxima robustez.
    """
    productos = []
...
async def api_arbol():
    """
    BUGFIX: obtener_nodos_visualizacion() devuelve dicts con clave 'nombre'.
    Nos aseguramos de que cada nodo tenga todos los campos que espera el JS.
    """
    ...
    for n in nodos_raw:
        nodos.append({
            "id":     n.get("id"),
            "padre":  n.get("padre"),        # None para la raíz
            "nivel":  n.get("nivel", 0),
            "pos":    n.get("pos", 0),
            "fe":     n.get("fe", 0),
            "nombre": n.get("nombre", ""),   # ← campo que avl_visualizer.js necesita
        })
    return {"nodos": nodos}
    ...
    if frente:
        return {
            "pedido": {
                ...
                "tipo":     frente.tipo,          # BUGFIX: Pedido usa .tipo, no .tipo_producto
...
async def api_despachar_pedido(request: Request):
    ...
    if dron.bateria <= 20:
        return {"status": "error", "message": "Batería insuficiente."}
    ...
    dron.ruta_nodos_latlng = [
        (sis.mapa.nodos[n].lat, sis.mapa.nodos[n].lon, n)
        for n in ruta
    ]
