...
# Es el único punto de contacto entre la GUI y la lógica
...
# Modelos disponibles para nuevos drones
MODELOS_DISPONIBLES = ['DJI Pro X', 'Phantom 4', 'Mavic 3', 'Agras T40', 'FlyCart 30']


class Sistema:
    def __init__(self):
        # Mapa aéreo de Santa Marta
        ...
        # Contador para IDs de drones (se determina del máximo existente)
        self._contador_drones = 5   # los 4 iniciales son D-01..D-04

        # Historial de entregas completadas
        self.entregas_completadas = []

    # ── FLOTA E INVENTARIO INICIALES ─────────────────────────────────────────

    def _inicializar_flota(self):
        flota_inicial = [
            ('D-01', 'DJI Pro X',  5.0, 80.0),
            ('D-02', 'DJI Pro X',  5.0, 80.0),
            ('D-03', 'Phantom 4',  3.0, 65.0),
            ('D-04', 'Phantom 4',  3.0, 65.0),
        ]
        posiciones = [(2, 2), (2, 4), (2, 6), (2, 8)]
        for (id_d, modelo, cap, vel), (f, c) in zip(flota_inicial, posiciones):
            d = Dron(id_d, modelo, cap, vel)
            d.coord_fila = f
            d.coord_col = c
            self.drones[id_d] = d
        ...
        self.drones['D-01'].registrar_mantenimiento(
            'Limpieza de salitre', 'Carlos M.', '2026-05-20', 'Exposición brisa marina')
        self.drones['D-01'].registrar_mantenimiento(
            'Cambio de batería',   'Luis R.',   '2026-05-15', 'Batería al 10% en vuelo')
        self.drones['D-02'].registrar_mantenimiento(
            'Calibración GPS',     'Ana G.',    '2026-05-18', 'Desvío detectado')

        # D-03: batería baja → pasa a mantenimiento automático
        self.drones['D-03'].bateria = 18
        self.drones['D-03']._evaluar_estado_bateria()

        # D-04: ya estaba en mantenimiento programado
    ...
    def _cargar_inventario_inicial(self):
        productos = [
            ...
            Producto(80,  'Batería DJI Pro',    'Repuesto',     5,  0.4),
        ]
        for p in productos:
    ...
    def agregar_dron(self, modelo, capacidad_kg=5.0, velocidad_kmh=80.0):
        """
        Crea un nuevo dron, le asigna ID correlativo y lo coloca
        en una posición libre de la matriz aérea.
        Retorna el dron creado o None si la flota está llena (>20 drones).
        """
        if len(self.drones) >= 20:
            return None, 'Capacidad máxima de la flota alcanzada (20 drones)'

        id_dron = f'D-{self._contador_drones:02d}'
        self._contador_drones += 1

        dron = Dron(id_dron, modelo, capacidad_kg, velocidad_kmh)

        # Busca posición libre en la primera fila disponible
        fila, col = self._posicion_libre_matriz()
        dron.coord_fila = fila
        dron.coord_col = col
        self.drones[id_dron] = dron
        self.espacio_aereo.insertar(fila, col, id_dron)

        return dron, f'Dron {id_dron} ({modelo}) incorporado a la flota'

    def _posicion_libre_matriz(self):
        """Busca la primera celda libre en la grilla 20×20."""
        for f in range(20):
            for c in range(20):
                if not self.espacio_aereo.esta_ocupada(f, c):
                    return f, c
        return 0, 0   # fallback (no debería ocurrir con ≤20 drones)

    def eliminar_dron(self, id_dron):
        """Elimina un dron de la flota (solo si no está en vuelo)."""
        if id_dron not in self.drones:
            return False, 'Dron no encontrado'
        dron = self.drones[id_dron]
        if dron.estado == 'en_vuelo':
            return False, f'No se puede eliminar {id_dron}: está en vuelo'
        self.espacio_aereo.eliminar(dron.coord_fila, dron.coord_col)
        del self.drones[id_dron]
        return True, f'Dron {id_dron} eliminado de la flota'
    ...
    def drones_en_mantenimiento(self):
        """Retorna los drones que requieren mantenimiento (batería ≤ 20 % o estado=mantenimiento)."""
        return [d for d in self.drones.values() if d.necesita_mantenimiento()]
    ...
    def despachar_siguiente(self):
        ...
        if dron_libre is None:
            return None, 'No hay drones disponibles (todos en vuelo, batería baja o en mantenimiento)'
        ...
        if ruta is None:
            self.cola_pedidos.encolar(pedido)
            return None, f'No se encontró ruta hacia {pedido.destino_nombre}'

        ok = dron_libre.asignar_pedido(pedido, ruta)
        if not ok:
            self.cola_pedidos.encolar(pedido)
            return None, f'{dron_libre.id_dron} requiere mantenimiento antes de volar'
    ...
    def _buscar_dron_disponible(self):
        for dron in self.drones.values():
            if dron.estado == 'en_espera' and dron.bateria > Dron.UMBRAL_BATERIA_CRITICA:
                return dron
        return None
    ...
    def mover_dron_matriz(self, id_dron, nueva_fila, nueva_col):
        ...
        return False, 'Colisión detectada o coordenadas inválidas'
