...
# v6: Rutas visuales corregidas — Ziruma rodeado con doble waypoint por el oeste
...
# ─────────────────────────────────────────────────────────────────────────────
# RUTAS VISUALES SEGURAS — evitan Cerro Ziruma y Zona Restringida Taganga
# Verificadas matemáticamente: todos los segmentos quedan fuera del radio
# de exclusión (0.6 km) de ambas zonas.
# ─────────────────────────────────────────────────────────────────────────────

# ── Waypoint para evitar Zona Taganga (11.2580, -74.1950) ───────────────────
# Desvío al noroeste, bordeando la zona por el flanco occidental
WP_TAGANGA_OESTE = (11.2640, -74.2210, "WP_TAG_O")

# ── Waypoints para evitar Cerro Ziruma (11.2190, -74.2270) ──────────────────
# El cerro queda justo en la línea recta ALMACEN → BELLO.
# Se necesitan DOS waypoints consecutivos para rodearlo por el oeste:
#   WP_ZIRUMA_NO — noroeste del cerro: aleja la trayectoria hacia el litoral
#   WP_ZIRUMA_SO — suroeste del cerro: desciende bordeando la costa hasta BELLO
# Todos los segmentos resultantes quedan a > 0.79 km de Ziruma (radio = 0.6 km).
...
def _build_rutas_visuales():
    ...
    # ── Corredor Norte: evita Zona Restringida Taganga ──────────────────────
    ...
    # ── Corredor Ziruma: ALMACEN ↔ BELLO — rodeo doble por el oeste ─────────
    # El cerro (11.2190,-74.2270) intercepta la línea directa entre ambos nodos.
    # WP_ZIRUMA_NO → WP_ZIRUMA_SO describe un arco costero que lo bordea:
    #   seg ALMACEN  → WP_NO: 0.79 km de Ziruma  ✅
    #   seg WP_NO    → WP_SO: 1.97 km de Ziruma  ✅
    #   seg WP_SO    → BELLO: 1.05 km de Ziruma  ✅
    rutas[("ALMACEN", "BELLO")] = [
        _wp("ALMACEN"), WP_ZIRUMA_NO, WP_ZIRUMA_SO, _wp("BELLO")
    ]
    rutas[("BELLO", "ALMACEN")] = [
        _wp("BELLO"), WP_ZIRUMA_SO, WP_ZIRUMA_NO, _wp("ALMACEN")
    ]

    # ── Rutas directas (verificadas sin zonas en el trayecto) ───────────────
    for par in [
...
@app.get("/api/drones")
async def api_listar_drones(): return {"drones": [_serializar_dron(d) for d in sis.lista_drones()]}

@app.get("/api/drones/mantenimiento")
async def api_drones_mantenimiento(): return {"drones": [_serializar_dron(d) for d in sis.drones_en_mantenimiento()]}

@app.get("/api/drones/modelos")
async def api_modelos(): return {"modelos": MODELOS_DISPONIBLES}
...
async def api_volar(id_dron: str): pass
...
async def api_registrar_mantenimiento(id_dron: str, request: Request):
    ...
    return {"status": "success", "message": f"Mantenimiento apilado", "estado_actual": dron.estado if dron else "?", "necesita_mant": dron.necesita_mantenimiento() if dron else False}
...
async def api_completar_entrega(id_dron: str):
    ...
    if dron: dron.status_ui = dron.estado
...
async def api_inventario_lista(): return {"productos": [{"id": p.id_producto, "nombre": p.nombre, "stock": p.stock, "peso": getattr(p, "peso_kg", getattr(p, "peso", 0))} for p in sis.lista_productos()]}

@app.get("/api/inventario/arbol")
async def api_arbol(): return {"nodos": [{"id": n.get("id"), "padre": n.get("padre"), "nivel": n.get("nivel", 0), "pos": n.get("pos", 0), "fe": n.get("fe", 0), "nombre": n.get("nombre", "")} for n in sis.inventario.obtener_nodos_visualizacion()]}
...
async def api_crear_pedido(request: Request):
    data = await request.json()
    sis.crear_pedido(data["destino"], (sis.mapa.nodos[data["destino"]].nombre if data["destino"] in sis.mapa.nodos else "Desconocido"), data["tipo"], data["prioridad"], float(data["peso"]))
...
async def api_pedido_frente():
    ...
    return {"pedido": {"id": f.id_pedido, "destino": f.destino_nombre, "tipo": f.tipo, "prioridad": f.prioridad, "peso": f.peso_kg} if f else None}
...
async def api_despachar_pedido(request: Request):
    ...
    if sis.cola_pedidos.esta_vacia(): return {"status": "error", "message": "Cola vacía."}
    dron = sis.drones.get(id_dron)
    if not dron: return {"status": "error", "message": "Dron no encontrado."}
    if dron.necesita_mantenimiento(): return {"status": "error", "message": f"{id_dron} requiere mant."}

    if partida and partida in sis.mapa.nodos:
        if sis.mapa.nodos[partida].tipo == 'excluido': return {"status": "error", "message": "Punto excluido."}
        dron.posicion_actual = partida
        if partida in _C: dron.lat, dron.lng = _C[partida]
    else: partida = dron.posicion_actual
    ...
    return {"status": "success", "message": f"Despacho exitoso: {id_dron}\nRuta: [ {' ➔ '.join(ruta)} ]"}
    ...
    try:
        while True:
            ...
            for dron in sis.lista_drones():
                if dron.estado not in ("en_vuelo", "retornando"): continue
                if not getattr(dron, "esperando_retorno", False): dron.consumir_bateria(0.2)
                ...
                if ruta_visual and not getattr(dron, "esperando_retorno", False):
                    if target_idx < len(ruta_visual):
                        ...
                        if dist < 0.002:
                            ...
                            if t_etiq in sis.mapa.nodos: dron.posicion_actual = t_etiq
                        else:
                            dron.lat += ((t_lat - dron.lat) / dist) * 0.002
                            dron.lng += ((t_lng - dron.lng) / dist) * 0.002
                    else:
                        if dron.estado == "en_vuelo": dron.esperando_retorno, dron.status_ui = True, "ESPERANDO RETORNO"
                        elif dron.estado == "retornando":
                            sis.completar_entrega(dron.id_dron)
                            dron.status_ui = dron.estado
                            if hasattr(dron, "ruta_visual"): del dron.ruta_visual

                if dron.estado in ("en_vuelo", "retornando"):
                    activos.append({"id": dron.id_dron, "lat": dron.lat, "lng": dron.lng, "battery": int(dron.bateria), "status": getattr(dron, "status_ui", dron.estado), "necesita_mant": dron.necesita_mantenimiento()})
