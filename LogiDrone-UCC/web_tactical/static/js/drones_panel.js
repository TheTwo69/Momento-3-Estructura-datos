// drones_panel.js — LogiDrone-UCC v5
// Gestión de flota: agregar drones, alertas de mantenimiento por batería ≤ 20%
'use strict';
...
function estadoColor(d) {
  ...
  const map = {
    en_vuelo: DC.cyan, en_espera: DC.blue,
    bateria_baja: DC.orange, mantenimiento: DC.red,
  };
...
function actualizarPantallasDrones() {
  fetch('/api/drones').then(r => r.json()).then(data => {
    ...
    // Tabla de la vista original (compatibilidad con map.js)
...
function abrirFormMant(idDron) {
  ...
  // Mostrar alerta si batería crítica
  fetch(`/api/drones`).then(r => r.json()).then(data => {
...
window.actualizarPantallasDrones = actualizarPantallasDrones;
